...
export default {
    ...
    mounted() {
        ...
        this.brandName = this.editProduct.brand_id;
        this.model = this.editProduct.model;
        this.stock = this.editProduct.stock;
        this.price = this.editProduct.price;
        this.sale_price = this.editProduct.sale_price;

        // Initialize productImages if there are existing images
        if (this.editProduct.image_paths) {
            this.productImages = this.editProduct.image_paths.map(imagePath => ({
                url: imagePath,
                file: null,
            }));
        }

        // Fetch brands based on the initial category
        if (this.category !== '') {
        ...
        previewFile(event) {
            ...
            if (files && files.length > 0) {
                ...
                for (let i = 0; i < files.length; i++) {
                    this.productImages.push({ file: files[i], url: null });
                }
            }
        },
        submitProductForm() {
            ...
            this.productImages.forEach((image, index) => {
                if (image.file) {
                    formData.append(`image_path_${index}`, image.file);
                }
            });
                    ...
                    <div class="mb-4">
                        ...
                        <div v-if="productImages.length" class="mt-4 grid grid-cols-2 gap-4">
                            <div v-for="(image, index) in productImages" :key="index" class="relative">
                                <img :src="image.file ? getObjectURL(image.file) : image.url" alt="Selected Image Preview" width="200">
                            </div>
                        </div>
                    </div>
                </div>
